...
function get_grid_version()
  ...
  if(grid_version<0) then
    if(field_exist(grid_file, 'geolon_t')) then
       ...
       call mpp_error(FATAL, module_name//'/get_grid_version: Can''t determine the version of the grid spec: none of' &
               //'"x_T", "geolon_t", or "ocn_mosaic_file" exist in file "'// trim(grid_file)//'"')
  ...
  case(VERSION_0,VERSION_1)
     ...
     case default
        call mpp_error(FATAL, module_name// &
                '/get_grid_cell_area: Illegal component name "'//trim(component)//'": must be one of ATM, LND, or OCN')
