...
!***********************************************************************
! turns on debug output (mostly for yaml output routines)
#ifndef DEBUG_YAML
#define DEBUG_YAML .false.
#endif
...
type diagYamlFiles_type
  ...
  character (len=MAX_STR_LEN), allocatable :: file_varlist(:)            !< An array of variable names
                                                                         !! within a file
  character (len=MAX_STR_LEN), allocatable :: file_outlist(:)            !< An array of output variable names
                                                                         !! within a file, only used for yaml output to distinguish fields
...
subroutine fms_diag_yaml_out()
  ...
  integer :: anum
  character(len=256) :: attribute_list
  character(len=MAX_STR_LEN), allocatable :: attribs(:,:)
  ...
  do i=1, SIZE(diag_yaml%diag_files)
    ...
    if( SIZE(fileptr%file_varlist) .gt. 0) then
      do j=1, SIZE(fileptr%file_varlist)
        ...
        print *, "searching for field:", fileptr%file_varlist(j)
        !! we have the yaml file object and list of fields, need to get the correct variable object for output
        nullify(varptr)
        !allocate(field_written(SIZE(diag_yaml%diag_fields))); field_written = .false.
        do varnum_i=1, SIZE(diag_yaml%diag_fields)
          if( trim(diag_yaml%diag_fields(varnum_i)%var_varname ) .eq. trim(fileptr%file_varlist(j)) .and. &
              trim(diag_yaml%diag_fields(varnum_i)%var_fname) .eq. trim(fileptr%file_fname) .and. &
              trim(diag_yaml%diag_fields(varnum_i)%var_outname) .eq. trim(fileptr%file_outlist(j))) then 

            if(DEBUG_YAML) print *, 'match', j, varnum_i
            print *, "long names:", trim(diag_yaml%diag_fields(varnum_i)%var_longname), "outlist", trim(fileptr%file_outlist(j))
            print *, "long names:", trim(diag_yaml%diag_fields(varnum_i)%var_outname), "outlist", trim(fileptr%file_outlist(j))
            varptr => diag_yaml%diag_fields(varnum_i)
            print *, "outname:", varptr%var_outname
            exit
          endif
        enddo
        ...
        call fms_f2c_string(keys3(key3_i)%key11, 'attributes')
        ...
        tmpstr1 = ''
        tmpstr1 = string(varptr%var_zbounds(1))
        tmpstr2 = string(varptr%var_zbounds(2))
        call fms_f2c_string(vals3(key3_i)%val8, trim(tmpstr1)//", "//trim(tmpstr2))
        ...
        if(varptr%has_var_attributes()) then
          attribute_list = ""
          attribs = varptr%get_var_attributes()
          do anum=1, size(varptr%get_var_attributes(), 1)
            attribute_list = trim(attribute_list)//" "//trim(attribs(anum,1))//":"//trim(attribs(anum,2))
          enddo
          call fms_f2c_string(vals3(key3_i)%val11, trim(adjustl(attribute_list)))
        endif
  ...
  if (DEBUG_YAML) print *, 'tier1size', 1, 'tier2size', SIZE(diag_yaml%diag_files), &
