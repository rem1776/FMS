program test_diag_bufer
    ...
    type(buffer0d) :: buffobj0
    type(buffer1d) :: buffobj1
    type(buffer2d) :: buffobj2
    type(buffer3d) :: buffobj3
    type(buffer4d) :: buffobj4
    type(buffer5d) :: buffobj5
    ...
    buff_id = buffobj0%allocate_buffer(r8_data)
    ! init to given value 
    call buffobj0%initialize_buffer( real(0.1, kind=r8_kind) )
    ! get the buffer
    buffer_out = buffobj0%get_buffer()  
    ! get the remapped buffer
    remap_buffer_out => buffobj0%get_remapped_buffer_pointer()
    ! check output
