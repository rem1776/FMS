...
contains
  ...
  subroutine create_exchange_test_input()

    call create_grid_spec()
    call create_atmos_mosaic()
    call create_atmos_hgrid(288, 180)
    call create_land_mosaic()
    call create_land_hgrids(180, 180)
    call create_sst_ice_clim(360, 180, 180, 91)

  end subroutine create_exchange_test_input

  subroutine create_data_override_test_input(atmos_nx, atmos_ny, land_nx, land_ny, data_nx, data_ny, ice_nx, ice_ny)
    integer, intent(in) :: atmos_nx, atmos_ny
    integer, intent(in) :: land_nx, land_ny
    integer, intent(in) :: data_nx, data_ny
    integer, intent(in) :: ice_nx, ice_ny


    call create_grid_spec()
    call create_atmos_mosaic()
    call create_atmos_hgrid(atmos_nx, atmos_ny)
    call create_land_mosaic()
    call create_land_hgrids(land_nx, land_ny)
    call create_sst_ice_clim(data_nx, data_ny, ice_nx, ice_ny)

  end subroutine create_data_override_test_input
  ...
  subroutine create_atmos_mosaic()
    ...
    if (open_file(fileobj, 'INPUT/atmos_mosaic.nc', 'overwrite')) then
  ...
  subroutine create_atmos_hgrid(grid_nx, grid_ny)
    integer, intent(in) :: grid_nx, grid_ny
    type(FmsNetcdfFile_t) :: fileobj
    ...
    if (open_file(fileobj, 'INPUT/atmos_hgrid.nc', 'overwrite')) then
  ...
  subroutine create_land_mosaic()
    ...
    if (open_file(fileobj, 'INPUT/unit_test_land_mosaic.nc', 'overwrite')) then
  ...
  subroutine create_land_hgrids(land_nx, land_ny)
    integer, intent(in) :: land_nx, land_ny
    type(FmsNetcdfFile_t) :: fileobj
    real, allocatable :: x(:,:), y(:,:)
    ...
    if (open_file(fileobj, 'INPUT/unit_test_land_hgrid.nc', 'overwrite')) then
    ...
    do tile = 1, 6
      ...
      if (open_file(fileobj, trim(filename), 'overwrite')) then
        call register_axis(fileobj, 'nx', land_nx)
        call register_axis(fileobj, 'ny', land_ny)
        call register_axis(fileobj, 'nxp', land_nx+1)
        call register_axis(fileobj, 'nyp', land_ny+1)
  ...
  subroutine create_sst_ice_clim(data_nx, data_ny, ice_nx, ice_ny)
    integer, intent(in) :: data_nx, data_ny, ice_nx, ice_ny
    type(FmsNetcdfFile_t) :: fileobj
    real, allocatable :: sst(:,:,:), sic(:,:,:), sit(:,:,:)
    double precision, allocatable :: lon(:), lat(:), time(:), lon_ice(:), lat_ice(:)
    integer :: i, t

    allocate(lon(data_nx))
    allocate(lat(data_ny))
    allocate(lon_ice(ice_nx))
    allocate(lat_ice(ice_ny))
    allocate(time(12))
    allocate(sst(data_nx, data_ny, 12))
    allocate(sic(data_nx, data_ny, 12))
    allocate(sit(ice_nx, ice_ny, 12))

    do i = 1, data_nx
      lon(i) = real(i-1) * 360.0 / data_nx - 180.0
    end do
    do i = 1, data_ny
      lat(i) = real(i-1) * 180.0 / data_ny - 90.0
    end do
    do i = 1, ice_nx
      lon_ice(i) = real(i-1) * 360.0 / ice_nx - 180.0
    end do
    do i = 1, ice_ny
      lat_ice(i) = real(i-1) * 180.0 / max(ice_ny - 1, 1) - 90.0
    end do
    do t = 1, 12
    ...
    if (open_file(fileobj, 'INPUT/sst_ice_clim.nc', 'overwrite')) then
      call register_axis(fileobj, 'LONGITUDE', data_nx)
      call register_axis(fileobj, 'LATITUDE', data_ny)
      call register_axis(fileobj, 'LON_ICE', ice_nx)
      call register_axis(fileobj, 'LAT', ice_ny)
...
end module create_test_input_mod
