...
program test_global_arrays
  ...
  do i=isc, iec
    do j=jsc, jec
      dataI4_5d(j, i, 1, 1, 1) = dataI4(j, i)
      dataI8_5d(j, i, 1, 1, 1) = dataI8(j, i)
    end do
  end do
  call mpp_sync()

  call mpp_error(NOTE, "----------Testing integer mpp_global_sum with 5 ranks and reordering----------")
  ...
  if( sumI4 .ne. sumI4_5d ) then
    call mpp_error(FATAL,"test_global_arrays: invalid 32-bit answer after reordering"// &
                        NEW_LINE('a')//"Sum: "// strTmp1 )
  endif
  ...
  if( sumI8 .ne. sumI8_5d) then
    call mpp_error(FATAL,"test_global_arrays: invalid 64-bit answer after reordering"// &
                        NEW_LINE('a')//"Sum: "// strTmp1 )
  endif
