    ...
    do elem = 1,NELEMS_report
       ...
       if(mpp_pe()==mpp_root_pe()) then
          ...
          write(stocks_file,formatString) 'Error =Diff/S(0)          ','[NonDim]    ', &
               ((f_value(i) - c_value(i))/(1+q_start(i)), i=1,NCOMPS)  !added 1 to avoid div
                                                                       !! by zero. Assuming q_start large


          write(stocks_file,'(a)'  ) ' '!blank line
          formatString = '(a,a,a,6x,es22.15)'
          write(stocks_file,formatString) 'Lost Stocks in the exchange between Ice and Ocean ', &
                  trim(STOCK_NAMES(elem)),trim(STOCK_UNITS(elem)),  &
