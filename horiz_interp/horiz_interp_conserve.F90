...
802    format (/,2x,'j',4x,'js',5x,'je',4x,'facjs',4x,'facje',  &
  ...
  subroutine horiz_interp_conserve_new_1dx2d ( Interp, lon_in, lat_in, lon_out, lat_out, &
    ...
    if( .not. great_circle_algorithm ) then
       if(flip_lat) then
          ...
          lon_in_r8 = lon_in
          lat_in_r8 = lat_in
          nxgrid = create_xgrid_1DX2D_order1(nlon_in, nlat_in, nlon_out, nlat_out, lon_in_r8, lat_in_r8, &
       ...
       if(flip_lat) then
          ...
          do j = 1, nlat_in
             mask_src_flip(:,j) = mask_src(:,nlat_in+1-j)
          enddo
          nxgrid = create_xgrid_great_circle(nlon_in, nlat_in, nlon_out, nlat_out, lon_src, lat_src, &
                                             lon_out_r8, lat_out_r8, mask_src_flip, i_src, j_src, i_dst, j_dst, &
                                             xgrid_area, clon, clat)
  ...
  subroutine horiz_interp_conserve_new_2dx2d ( Interp, lon_in, lat_in, lon_out, lat_out, &
    ...
    if( .not. great_circle_algorithm ) then
       nxgrid = create_xgrid_2DX2D_order1(nlon_in, nlat_in, nlon_out, nlat_out, lon_in_r8, lat_in_r8, &
                                          lon_out_r8, lat_out_r8, mask_src, i_src, j_src, i_dst, j_dst, xgrid_area)
    else
       allocate(clon(maxxgrid), clat(maxxgrid))
       nxgrid = create_xgrid_great_circle(nlon_in, nlat_in, nlon_out, nlat_out, lon_in_r8, lat_in_r8,   &
                                          lon_out_r8, lat_out_r8, mask_src, i_src, j_src, i_dst, j_dst, &
                                           xgrid_area, clon, clat)
    ...
    if(present(mask_out)) then
       ...
       mask_out = 0.0
       do i = 1, nxgrid
          mask_out(Interp%i_dst(i),Interp%j_dst(i)) = mask_out(Interp%i_dst(i), interp%j_dst(i)) + &
  ...
  subroutine horiz_interp_conserve ( Interp, data_in, data_out, verbose, &
    ...
    case (2)
       if(present(mask_in) .OR. present(mask_out) ) call mpp_error(FATAL, 'horiz_interp_conserve: for version 2'// &
                             ', mask_in and mask_out must be passed in horiz_interp_new, not in horiz_interp')
