...
! -- pjp --
 ...
 if ( seconds_new < 0 .or. ticks_new < 0) then
   call error_mesg('function set_time_i', &
...
! Computes time corresponding to date for gregorian calendar.
 ...
 if(yearx.gt.0) then
   ncenturies = int( yearx/100 )
   nlpyrs     = int( (yearx-ncenturies*100)/4 )
   dayx       = ncenturies*36524 + (yearx-ncenturies*100)*365 + nlpyrs ! 36524 days in 100
                                                                       !! years, year 100 not leap year
...
! Missing optionals are set to 0
 ...
 if(.not.allow_neg_inc_local) then
   if(oyears < 0 .or. omonths < 0 .or. odays < 0 .or. ohours < 0 .or. ominutes < 0 .or. &
      oseconds < 0 .or. oticks < 0) then
...
! Negative increments are always allowed in the private version of this routine.
 ...
 ! Convert this back into a time.
   select case(calendar_type)
   case(THIRTY_DAY_MONTHS)
     increment_date_private = set_date_thirty   (cyear, cmonth, cday, chour, cminute, csecond, ctick, Time_out, &
             err_msg)
